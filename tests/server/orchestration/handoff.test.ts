...
describe("redactSensitiveText", () => {
  // A URL is not a Windows drive path. The drive-letter rule used to match the
  // trailing letter of the scheme, rewriting every "https://x" the user typed
  // into "http[REDACTED PATH]" before it was ever persisted.
  it.each([
    "research https://en.wikipedia.org/wiki/Anthropocene give me 3 points",
    "research http://example.com/docs give me 3 points",
    "check https://api.github.com/repos/foo/bar?page=2",
    "ftp://host/file",
  ])("keeps the URL in %j intact", (input) => {
    expect(redactSensitiveText(input)).toBe(input);
  });

  it.each([
    ["see C:\\Users\\bob\\secret.txt", "C:\\Users"],
    ["path is D:/data/keys.txt", "D:/data"],
    ["env FOO=C:\\temp\\x", "C:\\temp"],
  ])("still redacts the Windows drive path in %j", (input, secret) => {
