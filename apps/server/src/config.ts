...
const envSchema = z.object({
  ...
  PROJECT_WORKSPACE_ROOT: z.string().default(path.resolve("project-workspaces")),
  ORCHESTRATION_QUEUE_LIMIT: z.coerce.number().int().min(1).max(1000).default(50),
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    projectWorkspaceRoot: path.resolve(env.PROJECT_WORKSPACE_ROOT),
    orchestrationQueueLimit: env.ORCHESTRATION_QUEUE_LIMIT,
